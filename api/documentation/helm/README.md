# berichtservice chart

The Helm chart installs berichtservice and by default the following dependencies using subcharts:
...
```bash
helm repo add berichtservice https://raw.githubusercontent.com/ConductionNL/berichtservice/master/api/helm/
helm repo update
```
...
```bash
helm install my-berichtservice ConductionNL/berichtservice --version 0.1.0
```
...
| `settings.subdomain` | the subdomain of the installation excluding www. | `berichtservice` |
| `settings.subpath` | Any sub path to follow the domain, like /api/v1 | `berichtservice` |
